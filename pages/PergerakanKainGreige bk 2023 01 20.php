...
STOCKOUT.PROVISIONALCODE,STOCKOUT.ORDERLINE
    ...
    while($rowdb23 = db2_fetch_assoc($stmt3)){ 
	$sqlDB2WRN = " 
SELECT ugp.LONGDESCRIPTION AS WARNA, p.PRODUCTIONORDERCODE ,LISTAGG(DISTINCT  TRIM(pd.CODE),', ') AS PRODUCTIONDEMANDCODE  
	...
	$tKKG31  +=$rowdb23['QTY_KG'];	
		...
		<?php 
		  //$sisaGreige=(round($tMKG,2)+$tMKGR+$tCGKG3In)-(round($tKKG31,2)+$tKCutS+$tKCutH+$tMKGTK); 
			$sisaGreige=(round($tMKG,2)+$tMKGR)-(round($tKKG31,2)+$tKCutS+$tKCutH); 
		  ?>	
		<div class="row">
			<div class="col-md-4">	
		<div class="card card-danger">
