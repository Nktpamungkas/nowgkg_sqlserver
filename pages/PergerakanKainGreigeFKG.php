...
SUBCODE02,SUBCODE03, SUM(BASEPRIMARYQUANTITY) AS BASEPRIMARYQUANTITY, CURRENT_TIMESTAMP AS TGLS FROM ITXVIEWHEADERKNTORDER 
WHERE (ITEMTYPEAFICODE ='FKF' OR ITEMTYPEAFICODE ='FKG') AND (PROGRESSSTATUS='2' OR PROGRESSSTATUS='6')
GROUP BY SUBCODE02,SUBCODE03,CURRENT_TIMESTAMP,(CASE WHEN PROJECTCODE <> '' THEN PROJECTCODE ELSE ORIGDLVSALORDLINESALORDERCODE  END)) X
...
WHERE a.ITEMTYPEAFICODE ='FKG' AND (a.PROJECTCODE ='$Project' OR a.ORIGDLVSALORDLINESALORDERCODE ='$Project') AND
		...
		<div class="row">
			<div class="col-md-3">
				<div class="card card-success">
					<div class="card-header">
						<h3 class="card-title">Filter Pergerakan Kain Greige FKG</h3>
			...
			<div class="col-md-6">
				<div class="card card-warning">
					<div class="card-header">
						<h3 class="card-title">Detail Data Kain Greige Masuk FKG</h3>
						<div class="card-tools">
							<button type="button" class="btn btn-tool" data-card-widget="collapse">
								<i class="fas fa-minus"></i>
							</button>
...
WHERE STOCKTRANSACTION.PROJECTCODE='$Project' AND DECOSUBCODE02='$subC1' AND DECOSUBCODE03='$subC2' AND
...
WHERE STOCKTRANSACTION.ITEMTYPECODE ='FKG'  AND STOCKTRANSACTION.LOGICALWAREHOUSECODE ='M021' 
...
WHERE s.PROJECTCODE='$Project' AND s.ITEMTYPECODE='FKG' AND 
...
s.LOTCODE='" . $rowdb21R['LOTCODE'] . "' AND s.TRANSACTIONDATE='" . $rowdb21R['TRANSACTIONDATE'] . "' AND
s.TEMPLATECODE = 'OPN' AND a.VALUESTRING =  '1'
GROUP BY 
s.ITEMELEMENTCODE
...
";
	...
	SELECT STOCKOUT.TRANSACTIONDATE,
	STOCKOUT.PROJECTCODE,STOCKOUT.CREATIONUSER,
	STOCKOUT.ORDERCODE,STOCKOUT.PROVISIONALCODE,
	COUNT(STOCKOUT.ITEMELEMENTCODE) AS QTY_ROL,
	SUM(STOCKOUT.BASEPRIMARYQUANTITY) AS QTY_KG
	FROM
(
SELECT 
STKKELUAR.ITEMELEMENTCODE,STKKELUAR.BASEPRIMARYQUANTITY,STKKELUAR.CREATIONUSER,  
       STKKELUAR.TRANSACTIONDATE,STKKELUAR.ORDERCODE,STKKELUAR.PROJECTCODE,STKKELUAR.LOTCODE, ITXVIEWLAPMASUKGREIGE.PROVISIONALCODE, 
...
AND NOT ITXVIEWLAPMASUKGREIGE.ORDERLINE IS NULL
GROUP BY STKKELUAR.ITEMELEMENTCODE,STKKELUAR.BASEPRIMARYQUANTITY,STKKELUAR.CREATIONUSER,  
       STKKELUAR.TRANSACTIONDATE,STKKELUAR.ORDERCODE, STKKELUAR.PROJECTCODE,STKKELUAR.LOTCODE,
	   ITXVIEWLAPMASUKGREIGE.PROVISIONALCODE,ITXVIEWLAPMASUKGREIGE.ORDERLINE) AS STOCKOUT
GROUP BY STOCKOUT.TRANSACTIONDATE,STOCKOUT.PROJECTCODE,STOCKOUT.ORDERCODE,STOCKOUT.CREATIONUSER,
...
LEFT OUTER JOIN DB2ADMIN.USERGENERICGROUP ugp ON
	pd.SUBCODE05 = ugp.CODE
LEFT OUTER JOIN DB2ADMIN.ITXVIEWCOLOR i ON
i.ITEMTYPECODE = pd.ITEMTYPEAFICODE
...
";
								...
								?>
									<tr>
										...
										<td style="text-align: center"><?php if ($rowdbWRN['PRODUCTIONDEMANDCODE'] != "") {
																			echo $rowdbWRN['PRODUCTIONDEMANDCODE'];
																		} else {
																			echo $rowdbPRJ['PRODUCTIONDEMANDCODE'];
																		} ?></td>
		...
		$sisaGreigeRol = ($tMRol + $tMRolSR + $tMRolRP + $tCSRol1) - ($tKRol31 + $tPRol);
		$sisaGreige = ($tMKG + $tMKGSR + $tMKGRP) - ($tKKG31 + $tKCutS + $tKCutH + $tPKG);
		?>
		<div class="row">
			<div class="col-md-4">
				<div class="card card-danger">
					<div class="card-header">
						<h3 class="card-title">Sisa Data Kain Greige CMD</h3>
						<div class="card-tools">
							<button type="button" class="btn btn-tool" data-card-widget="collapse">
								<i class="fas fa-minus"></i>
							</button>
