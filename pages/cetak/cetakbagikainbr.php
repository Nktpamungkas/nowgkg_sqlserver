...
GROUP BY
	...
	PRODUCTIONDEMAND.SUBCODE07,
	PRODUCTIONDEMAND.SUBCODE08";
$stmtPD   = db2_exec($conn1,$sqlPD, array('cursor'=>DB2_SCROLLABLE));
$rowPD = db2_fetch_assoc($stmtPD);

$itemKFF= trim($rowPD['SUBCODE01'])."-".trim($rowPD['SUBCODE02'])."-".trim($rowPD['SUBCODE03'])."-".trim($rowPD['SUBCODE04'])."-".trim($rowPD['SUBCODE05'])."-".trim($rowPD['SUBCODE06'])."-".trim($rowPD['SUBCODE07'])."-".trim($rowPD['SUBCODE08']);
$sqlBK="SELECT
...
WHERE
	...
	AND STOCKTRANSACTION.ITEMELEMENTCODE LIKE '".$rowdb2['DEMAND_KGF']."%'
LIMIT 1	
	";
	  $stmtPrjC   = db2_exec($conn1,$sqlPrjC, array('cursor'=>DB2_SCROLLABLE));	
      $rowPrjC   = db2_fetch_assoc($stmtPrjC);
if($rowPrj['PROJECTCODE']!=""){ $project=$rowPrj['PROJECTCODE'];}else{ $project=$rowPrjC['PROJECTCODE'];}
$sqlTGLBG="
SELECT
...
WHERE
    ...
    STOCKTRANSACTION.PROJECTCODE = '".$project."' AND
...
FROM
   ...
   WHERE i.ITEMTYPECODE='".$rowPD['ITEMTYPEAFICODE']."' AND
i.SUBCODE01 = '".$rowPD['SUBCODE01']."' AND
i.SUBCODE02 = '".$rowPD['SUBCODE02']."' AND
i.SUBCODE03 = '".$rowPD['SUBCODE03']."' AND
i.SUBCODE04 = '".$rowPD['SUBCODE04']."' AND
i.SUBCODE05 = '".$rowPD['SUBCODE05']."' AND
i.SUBCODE06 = '".$rowPD['SUBCODE06']."' AND
i.SUBCODE07 = '".$rowPD['SUBCODE07']."' AND
i.SUBCODE08 = '".$rowPD['SUBCODE08']."'     
";
  ...
  <table width="100%" border="0" style="font-size: 10px;">
    <tbody>
      ...
      <tr>
          ...
          <td width="1%" align="left" valign="top">:</td>
          <td width="16%" align="left" valign="top"><?php echo number_format(round($rowLG['WIDTHGROSS'],2),2)." x ".number_format(round($rowLG['VALUEDECIMAL'],2),2); ?></td>
      ...
      <tr>
          ...
          <td width="1%" align="center" valign="top">:</td>
          <td width="22%" align="left" valign="top"><?php echo $project;?></td>
          <td width="14%" align="right" valign="top">Mesin KNT</td>
          <td width="1%" align="left" valign="top">:</td>
          <td width="16%" align="left" valign="top"><?php echo $rowdb25['NO_MESIN']; ?></td>
