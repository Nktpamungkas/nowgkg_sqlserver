      ...
      <div class="container-fluid">
		...
		<div class="card card-success">
          <div class="card-header">
            <h3 class="card-title">Filter Identifikasi Kain Greige FKG</h3>
          ...
          <div class="card-body">
			...
			<div class="form-group row">
                    ...
                    <select name="hangerno" class="form-control form-control-sm"  autocomplete="off">
						...
						<option value="<?php echo trim($rowdb2['SUBCODE02']).trim($rowdb2['SUBCODE03']);?>" <?php if($HangerNO==trim($rowdb2['SUBCODE02']).trim($rowdb2['SUBCODE03'])){ echo "SELECTED";}?>><?php echo trim($rowdb2['SUBCODE02']).trim($rowdb2['SUBCODE03']);?></option>
						<?php } ?>
					</select>	
                  </div>	
                  </div>
		...
		<div class="card card-warning">
              <div class="card-header">
                <h3 class="card-title">Detail Data  Kain Greige FKG</h3>				 
          </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-sm table-bordered table-striped" style="font-size: 13px; text-align: center;">
                  <thead>
                  <tr>
                    <th colspan="18" valign="middle" style="text-align: center">Masuk</th>
                  ...
                  <tr>
                    ...
                    <th valign="middle" style="text-align: center">Lot Benang</th>
                    ...
                    <th valign="middle" style="text-align: center">Netto (PCS)</th>
                    <th valign="middle" style="text-align: center">Bruto (PCS)</th>
...
$c=0;
  ...
  SUM(STOCKTRANSACTION.BASESECONDARYQUANTITY) AS BRUTO,
  COUNT(STOCKTRANSACTION.BASEPRIMARYQUANTITY) AS QTY_ROL,
  a.VALUESTRING AS WARNA
FROM
  ...
  LEFT OUTER JOIN INTERNALDOCUMENTLINE i ON i.INTDOCUMENTPROVISIONALCODE = ITXVIEWLAPMASUKGREIGE.PROVISIONALCODE AND i.ORDERLINE =ITXVIEWLAPMASUKGREIGE.ORDERLINE
  LEFT OUTER JOIN ADSTORAGE a ON a.UNIQUEID = i.ABSUNIQUEID
  AND a.NAMENAME = 'NWarna'
WHERE
...
FROM
  (
    ...
    FROM
      ...
      ) AS STKBLANCE ON STKBLANCE.ELEMENTSCODE = STOCKTRANSACTION.ITEMELEMENTCODE
    WHERE
      STOCKTRANSACTION.ORDERCODE = '$rowdb21[PROVISIONALCODE]'
      and STOCKTRANSACTION.ORDERLINE = '$rowdb21[ORDERLINE]'
      AND STOCKTRANSACTION.LOGICALWAREHOUSECODE = 'M021'
      AND NOT ITXVIEWLAPMASUKGREIGE.ORDERLINE IS NULL
  ) AS BLKOKASI ";
...
FROM
  ...
  LEFT OUTER JOIN (
    ...
    WHERE
      ...
      AND QUALITYDOCUMENTITEMTYPEAFICODE = 'FKG'
...
FROM
  ...
  LEFT OUTER JOIN ADSTORAGE a ON a.UNIQUEID = e.ABSUNIQUEID
  AND a.NAMENAME = 'GSM'
...
?>
      ...
      <td style="text-align: center"><?php echo $rowdb25['NO_MESIN']; ?></td>
      ...
      <td style="text-align: right"><?php echo $rowdb21['EXTERNALREFERENCE']; ?></td>
      <td style="text-align: right"><?php echo round($rowdb21['BRUTO']); ?></td>
...
?>
      ...
      <td style="text-align: right"><?php echo $rowdb22R['WAREHOUSELOCATIONCODE']; ?></td>
      <td style="text-align: right"><?php if($rowdb22R['ROL']!=""){echo $rowdb22R['ROL'];}else{ echo"0";} ?></td>
      <td style="text-align: right"><?php if($rowdb22R['BERAT']!=""){echo $rowdb22R['BERAT'];}else{ echo"0.00";} ?></td>
	...
	$tKKGR +=$rowdb22R['BERAT'];	
	    ...
	    <td style="text-align: right"><strong><?php echo $tKKGR;?></strong></td>
	    <td>&nbsp;</td>
	    <td>&nbsp;</td>
	    </tr>			
		</tfoot>
    ...
    FROM
      ...
      INNER JOIN (
        ...
        FROM
          STOCKTRANSACTION
        WHERE
          STOCKTRANSACTION.ITEMTYPECODE = 'FKG'
          AND STOCKTRANSACTION.LOGICALWAREHOUSECODE = 'M021'
...
include "koneksi.php";		
$format = "20".date("ymd");
$sql=sqlsrv_query($con,"SELECT TOP 1 no_mutasi 
FROM dbnow_gkg.tbl_mutasi_kain 
...
$sql1=sqlsrv_query($con,"SELECT a.*, COUNT(b.transid) AS jmlrol, a.transid AS kdtrans 
FROM tbl_mutasi_kain a
LEFT JOIN tbl_prodemand b ON a.transid = b.transid
WHERE a.no_mutasi IS NULL 
AND CONVERT(VARCHAR(10), a.tgl_buat, 120) = $Awal 
AND a.gshift = $Gshift
GROUP BY a.transid, a.[other_columns]; 
");
$n1=1;
$noceklist1=1;	
while($r1=sqlsrv_fetch_array($sql1)){	
	if($_POST['cek'][$n1]!='') 
		...
		sqlsrv_query($con,"UPDATE dbnow_gkg.tbl_mutasi_kain SET
		no_mutasi='$nomid',
		tgl_mutasi=GETDATE()
