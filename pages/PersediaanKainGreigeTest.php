   ...
   $c=0;
	...
	$sqlDB21 = " SELECT
            ...
            FROM
              ...
              LEFT OUTER JOIN (
              SELECT PROJECTCODE, ORIGDLVSALORDLINESALORDERCODE, CODE FROM PRODUCTIONDEMAND
              )
              ...
              SELECT DISTINCT
			    ...
			    WHERE
			      p.ITEMTYPECODE IN ('KGF', 'FKG')) AS itmdesc ON itmdesc.SUBCODE02=b.DECOSUBCODE02 AND itmdesc.SUBCODE03=b.DECOSUBCODE03 AND itmdesc.SUBCODE04= b.DECOSUBCODE04
			   LEFT OUTER JOIN (
			   SELECT
                SALESORDER.CODE,
                SALESORDER.EXTERNALREFERENCE AS EXTERNALREFERENCE_LLG,
                SALESORDER.ORDPRNCUSTOMERSUPPLIERCODE,
                ITXVIEWAKJ.LEGALNAME1 AS LEGALNAME1_LLG,
                ITXVIEWAKJ.ORDERPARTNERBRANDCODE AS ORDERPARTNERBRANDCODE_LLG,
                ITXVIEWAKJ.LONGDESCRIPTION AS LONGDESCRIPTION_LLG
              FROM
                SALESORDER 
                LEFT OUTER JOIN ITXVIEWAKJ  ON SALESORDER.CODE = ITXVIEWAKJ.CODE
			   ) AS llg   ON llg.CODE = b.PROJECTCODE 	
			   LEFT OUTER JOIN (
			    SELECT
			        x.CODE AS LOTCODE,
	...
	else if($rowdb21['PROJECTCODE_PM']!=""){ 
		$proj=$rowdb21['PROJECTCODE_PM']; }
	else if($rowdb21['ORIGDLVSALORDLINESALORDERCODE_PM']!=""){ 
		$proj=$rowdb21['ORIGDLVSALORDLINESALORDERCODE_PM']; }
	else{ 
...
//	$rowdb22 = db2_fetch_assoc($stmt2);		
	if($rowdb21['LEGALNAME1_LLG']==""){$langganan="";}else{$langganan=$rowdb21['LEGALNAME1_LLG'];}
	if($rowdb21['ORDERPARTNERBRANDCODE_LLG']==""){$buyer="";}else{$buyer=$rowdb21['LONGDESCRIPTION_LLG'];}	
//	if($rowdb21['EXTERNALREFERENCE_LLG']!=""){
//		$PO=$rowdb21['EXTERNALREFERENCE_LLG'];
//	}else{
...
$arrayDesc = array_filter(array_map('trim', explode(',', $longdesc)));		
...
?>
      ...
      <td style="text-align: center"><?php  if($rowdb21['PROJAWAL']!=""){echo $rowdb21['PROJAWAL'];}else if($rowdb21['PROJAWAL1']!=""){echo $rowdb21['PROJAWAL1'];}else if($rowdb21['PROJECTCODE_PM']!=""){ echo $rowdb21['PROJECTCODE_PM']; }else if($rowdb21['ORIGDLVSALORDLINESALORDERCODE_PM']!=""){ echo $rowdb21['ORIGDLVSALORDLINESALORDERCODE_PM']; }else if($rowdb21['ORIGDLVSALORDLINESALORDERCODE']!=""){ echo $rowdb21['ORIGDLVSALORDLINESALORDERCODE'];}else{ echo $rowdb21['LOTCODE']; }  ?></td>
      <td style="text-align: center"><?php echo $jns; ?></td>
      <td style="text-align: center"><?php echo $itemNo; ?></td>
      <td style="text-align: center"><?php echo $rowdb21['SUMMARIZEDDESCRIPTION_ITEM']; ?></td>
      <td style="text-align: left"><?php echo $arrayDesc[0] ?? ''; ?></td>
      <td style="text-align: left"><?php echo $arrayDesc[1] ?? ''; ?></td>
      <td style="text-align: left"><?php echo $arrayDesc[2] ?? ''; ?></td>
      <td style="text-align: left"><?php echo $arrayDesc[3] ?? ''; ?></td>
