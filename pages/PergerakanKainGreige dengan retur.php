...
WHERE a.ITEMTYPEAFICODE ='KGF' AND a.PROJECTCODE ='$Project' AND
...
?>
	...
	$tMKG +=$rowdb21['QTY_KG'];		
		...
		<div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Detail Data Kain Greige Return Production</h3>				 
          </div>
              <!-- /.card-header -->              		
					<div class="card-body">
...
?>
      ...
      <td style="text-align: right"><?php echo number_format(round($skg1,2),2); ?></td>
      </tr>
...
?>
	...
	$tKCutH +=$tCH;	
		...
		<div class="card card-danger">
              <div class="card-header">
                <h3 class="card-title">Data Kain Greige Keluar</h3>				 
          </div>
              <!-- /.card-header -->              		
					<div class="card-body">
...
INNER JOIN (SELECT
	...
	STOCKTRANSACTION.ITEMELEMENTCODE
FROM STOCKTRANSACTION 
LEFT OUTER JOIN DB2ADMIN.PRODUCTIONDEMANDSTEP PRODUCTIONDEMANDSTEP ON
   PRODUCTIONDEMANDSTEP.PRODUCTIONORDERCODE = STOCKTRANSACTION.ORDERCODE
   AND PRODUCTIONDEMANDSTEP.ITEMTYPEAFICODE = 'KFF' 
   AND PRODUCTIONDEMANDSTEP.PLANNEDWORKCENTERCODE ='P3GR1'
WHERE STOCKTRANSACTION.PROJECTCODE='$Project' AND DECOSUBCODE02='$subC1' AND DECOSUBCODE03='$subC2' AND DECOSUBCODE04='$subC3' 
AND NOT STOCKTRANSACTION.ORDERCODE IS NULL AND STOCKTRANSACTION.TEMPLATECODE ='120'
...
STOCKTRANSACTION.TRANSACTIONDATE,
	...
	STOCKTRANSACTION.PROJECTCODE,
	STOCKTRANSACTION.BASEPRIMARYQUANTITY,
	STOCKTRANSACTION.ITEMELEMENTCODE) AS STKKELUAR ON STKKELUAR.ITEMELEMENTCODE=STOCKTRANSACTION.ITEMELEMENTCODE
WHERE STOCKTRANSACTION.PROJECTCODE='$Project' AND DECOSUBCODE02='$subC1' AND DECOSUBCODE03='$subC2' AND DECOSUBCODE04='$subC3' and 
STOCKTRANSACTION.LOGICALWAREHOUSECODE ='M021' 
AND NOT ITXVIEWLAPMASUKGREIGE.ORDERLINE IS NULL) AS STOCKOUT
	...
	$tKKG3 +=$rowdb23['QTY_KG'];	
		...
		<div class="card card-danger">
              ...
              <!-- /.card-header -->              		
                  ...
                  <thead>
                  <tr>
                    <th colspan="7" valign="middle" style="text-align: center">Keluar</th>
                    </tr>
                  <tr>
                    <th valign="middle" style="text-align: center">Tanggal</th>
                    <th valign="middle" style="text-align: center">Prod. Order</th>
...
$c=0;
	...
	$sqlDB23 = " 
	 SELECT STKKELUAR.TRANSACTIONDATE,
 	   STKKELUAR.PROJECTCODE,
 	   STKKELUAR.ORDERCODE,
	   STKKELUAR.DECOSUBCODE02,
	   STKKELUAR.DECOSUBCODE03,
	   STKKELUAR.DECOSUBCODE04,
       ITXVIEWLAPMASUKGREIGE.LOTCODE,
       SUM(STKKELUAR.BASEPRIMARYQUANTITY) AS QTY_KG,
       COUNT(STKKELUAR.BASEPRIMARYQUANTITY) AS QTY_ROL
       FROM DB2ADMIN.STOCKTRANSACTION STOCKTRANSACTION LEFT OUTER JOIN
DB2ADMIN.ITXVIEWLAPMASUKGREIGE ITXVIEWLAPMASUKGREIGE ON ITXVIEWLAPMASUKGREIGE.PROVISIONALCODE  = STOCKTRANSACTION.ORDERCODE
AND ITXVIEWLAPMASUKGREIGE.ORDERLINE  = STOCKTRANSACTION.ORDERLINE
AND ITXVIEWLAPMASUKGREIGE.PROVISIONALCOUNTERCODE  = STOCKTRANSACTION.ORDERCOUNTERCODE  
...
INNER JOIN (SELECT
	...
	stk.ORDERCODE,
	stk.PROJECTCODE,
	stk.BASEPRIMARYQUANTITY,
	stk.ITEMELEMENTCODE,
	stk.DECOSUBCODE02,
	stk.DECOSUBCODE03,
	stk.DECOSUBCODE04
FROM STOCKTRANSACTION stk 
WHERE NOT stk.ORDERCODE IS NULL AND 
stk.TEMPLATECODE ='120' AND 
stk.LOGICALWAREHOUSECODE ='M021' 
GROUP BY 
    stk.TRANSACTIONDATE,
	stk.ORDERCODE,
	stk.PROJECTCODE,
	...
	stk.DECOSUBCODE02,
	stk.DECOSUBCODE03,
	stk.DECOSUBCODE04) AS STKKELUAR ON STKKELUAR.ITEMELEMENTCODE=STOCKTRANSACTION.ITEMELEMENTCODE
WHERE STOCKTRANSACTION.PROJECTCODE='$Project' AND 
STOCKTRANSACTION.DECOSUBCODE02='$subC1' AND 
STOCKTRANSACTION.DECOSUBCODE03='$subC2' AND 
STOCKTRANSACTION.DECOSUBCODE04='$subC3' and 
STOCKTRANSACTION.LOGICALWAREHOUSECODE ='M021' 
AND NOT ITXVIEWLAPMASUKGREIGE.ORDERLINE IS NULL
GROUP BY STKKELUAR.TRANSACTIONDATE,		
	   STKKELUAR.PROJECTCODE,
 	   STKKELUAR.ORDERCODE,
       ITXVIEWLAPMASUKGREIGE.LOTCODE,
       STKKELUAR.DECOSUBCODE02,
...
?>
	...
	$tKKG31  +=$rowdb23['QTY_KG'];	
	    ...
	    <td style="text-align: right"><strong><?php echo number_format(round($tKKG31,2),2);?></strong></td>
	    </tr>			
		</tfoot>
                </table>					
					</div> 
