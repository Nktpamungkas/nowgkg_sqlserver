...
$sqlDB210 = " 
SELECT SUM(a.BASEPRIMARYQUANTITY) AS BASEPRIMARYQUANTITY, SUM(a3.VALUEDECIMAL) AS QTYSALIN  FROM (
...
s.ITEMELEMENTCODE
		...
		DB2ADMIN.BALANCE BALANCE ON BALANCE.ELEMENTSCODE =STOCKTRANSACTION.ITEMELEMENTCODE  
		GROUP BY BALANCE.LOTCODE ";	*/
                  $sqlDB22R = " SELECT STOCKTRANSACTION.TRANSACTIONNUMBER, LISTAGG(DISTINCT  TRIM(BALANCE.WAREHOUSELOCATIONCODE),', ') AS WAREHOUSELOCATIONCODE, COUNT(BALANCE.BASEPRIMARYQUANTITYUNIT) AS ROL,SUM(BALANCE.BASEPRIMARYQUANTITYUNIT) AS BERAT,BALANCE.LOTCODE  
		FROM (
		SELECT 
...
s.DECOSUBCODE02='$subC1' AND s.DECOSUBCODE03='$subC2' AND 
s.DECOSUBCODE04='$subC3' AND s.LOGICALWAREHOUSECODE ='M021' AND
s.LOTCODE='$rowdb21R[LOTCODE]' AND s.TRANSACTIONNUMBER='$rowdb21R[TRANSACTIONNUMBER]' AND s.TRANSACTIONDATE='$rowdb21R[TRANSACTIONDATE]' AND 
s.TEMPLATECODE = 'OPN' AND a.VALUESTRING =  '1'
	) STOCKTRANSACTION LEFT OUTER JOIN 
		DB2ADMIN.BALANCE BALANCE ON BALANCE.ELEMENTSCODE =STOCKTRANSACTION.ITEMELEMENTCODE  
		GROUP BY BALANCE.LOTCODE, STOCKTRANSACTION.TRANSACTIONNUMBER ";
