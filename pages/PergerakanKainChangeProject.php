...
if ($_POST['mutasikain'] == "MutasiKain") {
	...
	function mutasiurut()
	{
		include "koneksi.php";
		$format = "20" . date("ymd");
		$sql = sqlsrv_query($con, "SELECT TOP 1 no_mutasi FROM dbnow_gkg.tbl_mutasi_kain WHERE SUBSTRING(no_mutasi,1,8) LIKE '%" . $format . "%' ORDER BY no_mutasi DESC", array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));
	...
	$sql1 = sqlsrv_query($con, "SELECT *,COUNT(b.transid) AS jmlrol,a.transid AS kdtrans FROM dbnow_gkg.tbl_mutasi_kain a 
			LEFT JOIN dbnow_gkg.tbl_prodemand b ON a.transid=b.transid 
			WHERE ISNULL(NULL,a.no_mutasi) AND CONVERT(date, a.tgl_buat) = '$Awal' AND a.gshift = '$Gshift' 
			GROUP BY a.transid");
	$n1 = 1;
	$noceklist1 = 1;
